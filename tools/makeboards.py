...
def BuildHeader(name, processor, vendor_name, product_name, vid, pid, pwr, boarddefine, variant, flashsize, boot2, extra):
    ...
    if processor == "rp2040":
        ...
        print("%s.build.architecture=-march=armv6-m -mthumb" % (name))
        print("%s.build.pico-sdk-switch=PICO_RP2040" % (name))
        print("%s.build.f_cpu=125000000" % (name))
        print("%s.build.pico-variant=rp2040" % (name))
    else:
        print("%s.build.mcu=cortex-m33" % (name))
        print("%s.build.architecture=-mthumb -march=armv8-m.main+fp+dsp -mfloat-abi=softfp" % (name))
        print("%s.build.pico-sdk-switch=PICO_RP2350" % (name))
        print("%s.build.f_cpu=150000000" % (name))
        print("%s.build.pico-variant=rp2350" % (name))
...
def BuildWifiType(name):
    ...
    print("%s.menu.wifitype.esp_at.build.wifi=-DWIFIESPAT2" % (name))
    print("%s.menu.wifitype.esp_hosted=ESP Hosted" % (name))
    print("%s.menu.wifitype.esp_hosted.build.wifi=-DESPHOSTSPI=SPI1" % (name))
...
def MakeBoard(name, processor, vendor_name, product_name, vid, pid, pwr, boarddefine, flashsizemb, boot2, extra = None, board_url = None):
    ...
    BuildHeader(name, processor, vendor_name, product_name, vid, pid, pwr, boarddefine, name, flashsizemb * 1024 * 1024, boot2, extra)
