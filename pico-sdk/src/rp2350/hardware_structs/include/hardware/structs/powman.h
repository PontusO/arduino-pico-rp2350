...
// Reference to datasheet: https://datasheets.raspberrypi.com/rp2040/rp2040-datasheet.pdf#tab-registerlist_powman
//
...
typedef struct {
    ...
    // 0x00000070 [6:4]   : HT_TH (0x5): high impedance mode select
    // 0x00000003 [1:0]   : LT_TH (0): enable
    io_rw_32 vreg_ctrl;
    ...
    // 0x000001f0 [8:4]   : VSEL (0xb): output voltage select
    // 0x00000004 [2]     : MODE (0): selects either normal (switching) mode or low power (linear) mode
    ...
    // 0x000001f0 [8:4]   : VSEL (0xb): output voltage select
    // 0x00000004 [2]     : MODE (1): selects either normal (switching) mode or low power (linear) mode
    // 0x00000002 [1]     : HIZ (0): high impedance mode select
    ...
    // 0x000001f0 [8:4]   : VSEL (0xb): threshold select
    ...
    // 0x000003f0 [9:4]   : TRIM (0x20): frequency trim
    // 0x00000003 [1:0]   : MODE (0x3): mode
    io_rw_32 lposc;

    _REG_(POWMAN_CHIP_RESET_OFFSET) // POWMAN_CHIP_RESET
    // Chip reset control and status
    // 0x10000000 [28]    : HAD_WATCHDOG_RESET_RSM (0): Last reset was a watchdog timeout which was configured to reset the...
    ...
    // 0x01000000 [24]    : HAD_WATCHDOG_RESET_SWCORE (0): Last reset was a watchdog timeout which was configured to reset...
    // 0x00800000 [23]    : HAD_WATCHDOG_RESET_POWMAN (0): Last reset was a watchdog timeout which was configured to reset...
    // 0x00400000 [22]    : HAD_WATCHDOG_RESET_POWMAN_ASYNC (0): Last reset was a watchdog timeout which was configured to...
    ...
    // 0x00000001 [0]     : DOUBLE_TAP (0): This flag is not reset by RUN
    io_rw_32 chip_reset;

    _REG_(POWMAN_WATCHDOG_OFFSET) // POWMAN_WATCHDOG
    // Allows a watchdog reset to reset the internal state of powman in addition to the reset state machine (PSM)
    // 0x00001000 [12]    : RESET_RSM (0): If set to 1, a watchdog reset will run the full reset state machine (RSM) sequence
    // 0x00000100 [8]     : RESET_SWCORE (0): If set to 1, a watchdog reset will reset the switched core power domain and...
    // 0x00000010 [4]     : RESET_POWMAN (0): If set to 1, a watchdog reset will restore powman defaults, reset the timer,...
    // 0x00000001 [0]     : RESET_POWMAN_ASYNC (0): If set to 1, a watchdog reset will restore powman defaults, reset the timer,
    io_rw_32 watchdog;

    _REG_(POWMAN_SEQ_CFG_OFFSET) // POWMAN_SEQ_CFG
    // For configuration of the power sequencer
    // 0x00100000 [20]    : USING_FAST_POWCK (1): 0 indicates the POWMAN clock is running from the low power oscillator (32kHz)
    ...
    // 0x00001000 [12]    : USE_FAST_POWCK (1): selects the reference clock (clk_ref) as the source of the POWMAN clock...
    // 0x00000100 [8]     : RUN_LPOSC_IN_LP (1): Set to 0 to stop the low power osc when the switched-core is powered down,...
    // 0x00000080 [7]     : USE_BOD_HP (1): Set to 0 to prevent automatic switching to bod high power mode when...
    // 0x00000040 [6]     : USE_BOD_LP (1): Set to 0 to prevent automatic switching to bod low power mode when...
    // 0x00000020 [5]     : USE_VREG_HP (1): Set to 0 to prevent automatic switching to vreg high power mode when...
    // 0x00000010 [4]     : USE_VREG_LP (1): Set to 0 to prevent automatic switching to vreg low power mode when...
    // 0x00000002 [1]     : HW_PWRUP_SRAM0 (0): Specifies the power state of SRAM0 when powering up swcore from a low power...
    // 0x00000001 [0]     : HW_PWRUP_SRAM1 (0): Specifies the power state of SRAM1 when powering up swcore from a low power...
    io_rw_32 seq_cfg;
    ...
    // 0x000000f0 [7:4]   : REQ (0)
    // 0x0000000f [3:0]   : CURRENT (0xf)
    io_rw_32 state;

    _REG_(POWMAN_POW_FASTDIV_OFFSET) // POWMAN_POW_FASTDIV
    // 0x000007ff [10:0]  : POW_FASTDIV (0x40): divides the POWMAN clock to provide a tick for the delay module and state machines
    ...
    // 0x000000f0 [7:4]   : XIP_STEP (1): timing between the xip power state machine steps
    // 0x0000000f [3:0]   : SWCORE_STEP (1): timing between the swcore power state machine steps
    io_rw_32 pow_delay;
    ...
    // 0x00000010 [4]     : DRIVE_LPCK (0): Use the selected GPIO to drive the 32kHz low power clock, in place of LPOSC
    // 0x00000003 [1:0]   : SOURCE_SEL (0): 0 ->  gpio12
    io_rw_32 ext_time_ref;

    _REG_(POWMAN_LPOSC_FREQ_KHZ_INT_OFFSET) // POWMAN_LPOSC_FREQ_KHZ_INT
    // 0x0000003f [5:0]   : LPOSC_FREQ_KHZ_INT (0x20): Integer component of the LPOSC or GPIO clock source frequency in kHz
    io_rw_32 lposc_freq_khz_int;

    _REG_(POWMAN_LPOSC_FREQ_KHZ_FRAC_OFFSET) // POWMAN_LPOSC_FREQ_KHZ_FRAC
    // 0x0000ffff [15:0]  : LPOSC_FREQ_KHZ_FRAC (0xc49c): Fractional component of the LPOSC or GPIO clock source frequency in kHz
    ...
    // 0x0000ffff [15:0]  : XOSC_FREQ_KHZ_FRAC (0): Fractional component of the XOSC frequency in kHz
    io_rw_32 xosc_freq_khz_frac;

    _REG_(POWMAN_SET_TIME_63TO48_OFFSET) // POWMAN_SET_TIME_63TO48
    // 0x0000ffff [15:0]  : SET_TIME_63TO48 (0): For setting the time, do not use for reading the time, use...
    io_rw_32 set_time_63to48;

    _REG_(POWMAN_SET_TIME_47TO32_OFFSET) // POWMAN_SET_TIME_47TO32
    // 0x0000ffff [15:0]  : SET_TIME_47TO32 (0): For setting the time, do not use for reading the time, use...
    io_rw_32 set_time_47to32;

    _REG_(POWMAN_SET_TIME_31TO16_OFFSET) // POWMAN_SET_TIME_31TO16
    // 0x0000ffff [15:0]  : SET_TIME_31TO16 (0): For setting the time, do not use for reading the time, use...
    io_rw_32 set_time_31to16;

    _REG_(POWMAN_SET_TIME_15TO0_OFFSET) // POWMAN_SET_TIME_15TO0
    // 0x0000ffff [15:0]  : SET_TIME_15TO0 (0): For setting the time, do not use for reading the time, use...
    io_rw_32 set_time_15to0;

    _REG_(POWMAN_READ_TIME_UPPER_OFFSET) // POWMAN_READ_TIME_UPPER
    // For reading bits 63:32 of the timer
    ...
    // 0x0000ffff [15:0]  : ALARM_TIME_63TO48 (0): This field must only be written when POWMAN_ALARM_ENAB=0
    io_rw_32 alarm_time_63to48;

    _REG_(POWMAN_ALARM_TIME_47TO32_OFFSET) // POWMAN_ALARM_TIME_47TO32
    // 0x0000ffff [15:0]  : ALARM_TIME_47TO32 (0): This field must only be written when POWMAN_ALARM_ENAB=0
    io_rw_32 alarm_time_47to32;

    _REG_(POWMAN_ALARM_TIME_31TO16_OFFSET) // POWMAN_ALARM_TIME_31TO16
    // 0x0000ffff [15:0]  : ALARM_TIME_31TO16 (0): This field must only be written when POWMAN_ALARM_ENAB=0
    io_rw_32 alarm_time_31to16;

    _REG_(POWMAN_ALARM_TIME_15TO0_OFFSET) // POWMAN_ALARM_TIME_15TO0
    // 0x0000ffff [15:0]  : ALARM_TIME_15TO0 (0): This field must only be written when POWMAN_ALARM_ENAB=0
    ...
    // 0x00000004 [2]     : CLEAR (0): Clears the timer, does not disable the timer and does not affect the alarm
    // 0x00000002 [1]     : RUN (0): Enables the timer, the timer is not cleared when RUN is deasserted
    // 0x00000001 [0]     : NONSEC_WRITE (0): Control whether NonSecure software can write to the timer registers
    io_rw_32 timer;
    ...
    // 0x0000007f [6:0]   : CURRENT_PWRUP_REQ (0)
    ...
    // 0x0000007f [6:0]   : LAST_SWCORE_PWRUP (0)
    ...
    // 0x00000001 [0]     : NOW (0): When powman resets the RSM, the current value of BOOTDIS_NEXT is OR'd into...
    io_rw_32 bootdis;

    _REG_(POWMAN_DBGCONFIG_OFFSET) // POWMAN_DBGCONFIG
    // 0x0000000f [3:0]   : DP_INSTID (0): Configure DP instance ID for SWD multidrop selection
    io_rw_32 dbgconfig;
