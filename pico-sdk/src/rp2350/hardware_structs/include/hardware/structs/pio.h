...
typedef struct pio_sm_hw {
    ...
    // 0x00000060 [6:5]   : STATUS_SEL (0): Comparison used for the MOV x, STATUS instruction
    // 0x0000001f [4:0]   : STATUS_N (0): Comparison level or IRQ index for the MOV x, STATUS instruction
    io_rw_32 execctrl;
    ...
    // 0x00008000 [15]    : FJOIN_RX_PUT (0): If 1, disable this state machine's RX FIFO, make its storage available for...
    // 0x00004000 [14]    : FJOIN_RX_GET (0): If 1, disable this state machine's RX FIFO, make its storage available for...
    // 0x0000001f [4:0]   : IN_COUNT (0): Set the number of pins which are not masked to 0 when read by an IN PINS, WAIT...
    io_rw_32 shiftctrl;
...
typedef struct {
    ...
    // 0x04000000 [26]    : NEXTPREV_CLKDIV_RESTART (0): Write 1 to restart the clock dividers of state machines in...
    // 0x02000000 [25]    : NEXTPREV_SM_DISABLE (0): Write 1 to disable state machines in neighbouring PIO blocks, as...
    // 0x01000000 [24]    : NEXTPREV_SM_ENABLE (0): Write 1 to enable state machines in neighbouring PIO blocks, as...
    // 0x00f00000 [23:20] : NEXT_PIO_MASK (0): A mask of state machines in the neighbouring higher-numbered PIO block in...
    // 0x000f0000 [19:16] : PREV_PIO_MASK (0): A mask of state machines in the neighbouring lower-numbered PIO block in the...
    ...
    // 0xf0000000 [31:28] : VERSION (1): Version of the core PIO hardware
    ...
    io_rw_32 rxf0_putget[4];

    _REG_(PIO_RXF1_PUTGET0_OFFSET) // PIO_RXF1_PUTGET0
    // (Description copied from array index 0 register PIO_RXF1_PUTGET0 applies similarly to other array indexes)
    //
    // Direct read/write access to entry 0 of SM1's RX FIFO, if SHIFTCTRL_FJOIN_RX_PUT xor SHIFTCTRL_FJOIN_RX_GET is set
    io_rw_32 rxf1_putget[4];

    _REG_(PIO_RXF2_PUTGET0_OFFSET) // PIO_RXF2_PUTGET0
    // (Description copied from array index 0 register PIO_RXF2_PUTGET0 applies similarly to other array indexes)
    //
    // Direct read/write access to entry 0 of SM2's RX FIFO, if SHIFTCTRL_FJOIN_RX_PUT xor SHIFTCTRL_FJOIN_RX_GET is set
    io_rw_32 rxf2_putget[4];

    _REG_(PIO_RXF3_PUTGET0_OFFSET) // PIO_RXF3_PUTGET0
    // (Description copied from array index 0 register PIO_RXF3_PUTGET0 applies similarly to other array indexes)
    //
    // Direct read/write access to entry 0 of SM3's RX FIFO, if SHIFTCTRL_FJOIN_RX_PUT xor SHIFTCTRL_FJOIN_RX_GET is set
    io_rw_32 rxf3_putget[4];
