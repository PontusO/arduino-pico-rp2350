...
enum clock_index {
    ...
    clk_hstx,           ///< Peripheral clock for HSTX
    clk_usb,            ///< USB clock
    clk_adc,            ///< ADC clock
    CLK_COUNT
};
...
typedef struct {
    ...
    // 0x00001000 [12]    : DC50 (0): Enables duty cycle correction for odd divisors, can be changed on-the-fly
    // 0x00000800 [11]    : ENABLE (0): Starts and stops the clock generator cleanly
    // 0x00000400 [10]    : KILL (0): Asynchronously kills the clock generator, enable must be set low before deasserting kill
    ...
    // 0xffff0000 [31:16] : INT (1): Integer part of clock divisor, 0 -> max+1, can be changed on-the-fly
    // 0x0000ffff [15:0]  : FRAC (0): Fractional component of the divisor, can be changed on-the-fly
    io_rw_32 div;

    _REG_(CLOCKS_CLK_GPOUT0_SELECTED_OFFSET) // CLOCKS_CLK_GPOUT0_SELECTED
    // Indicates which src is currently selected (one-hot)
    // 0x00000001 [0]     : CLK_GPOUT0_SELECTED (1): This slice does not have a glitchless mux (only the AUX_SRC field is...
    io_ro_32 selected;
} clock_hw_t;
/// \end::clock_hw[]
