...
// these are not more specific, because they are just an optimization when booting in either ARM or RISC-V
#define PICOBIN_PARTITION_FLAGS_NOT_BOOTABLE_ARM_BITS                       _u(0x00000200)
#define PICOBIN_PARTITION_FLAGS_NOT_BOOTABLE_RISCV_BITS                     _u(0x00000400)
...
// we have a bit for each well known family-id .. note we expet there to be more in the future with new chips,
...
static inline uint picobin_load_map_is_relative(const picobin_load_map *lm) {
