...
#endif
// todo amy remove me
static inline int running_in_sim(void) { return 0; }
...
/*! \brief Return true if called from NonSecure state (Arm-only)
 *  \ingroup pico_platform
 *
 * \return True if called from NonSecure state
 */
__force_inline static bool pico_processor_state_is_nonsecure(void) {
    // Armv6-M does not have the Security extension
    return false;
}
