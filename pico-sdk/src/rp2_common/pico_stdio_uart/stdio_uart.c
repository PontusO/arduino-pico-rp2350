...
#else
    if (tx_pin >= 0) gpio_set_function((uint)tx_pin,
        tx_pin & 0x2 ? GPIO_FUNC_UART_AUX : GPIO_FUNC_UART);
    if (rx_pin >= 0) gpio_set_function((uint)rx_pin,
        rx_pin & 0x2 ? GPIO_FUNC_UART_AUX : GPIO_FUNC_UART);
#endif
