...
#else
    if (enabled) {
        hw_set_bits(&pwm_hw->inte0, 1u << slice_num);
    } else {
        hw_clear_bits(&pwm_hw->inte0, 1u << slice_num);
    }
#endif
}
...
#else
    if (enabled) {
        hw_set_bits(&pwm_hw->inte0, slice_mask);
    } else {
        hw_clear_bits(&pwm_hw->inte0, slice_mask);
    }
#endif
}
