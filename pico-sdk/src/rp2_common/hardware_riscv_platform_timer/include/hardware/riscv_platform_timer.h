 ...
 *  \defgroup riscv_platform_timer riscv_platform_timer
 *
 * Accessors for standard RISC-V platform timer (mtime/mtimecmp), available on
...
static inline uint64_t riscv_timer_get_mtimecmp(void) {
    ...
    // mtimecmp increases monotonically with successive interrrupts.
    uint32_t h0, l, h1;
    do {
        h0 = sio_hw->mtimecmph;
        l  = sio_hw->mtimecmp;
