...
#if !defined(PICO_USE_GPIO_COPROCESSOR) && !PICO_RP2040 && !defined(__riscv)
...
static inline bool gpio_get(uint gpio) {
    ...
    if (gpio < 32) {
        ...
        return !!((1ul << (gpio - 32)) & sio_hw->gpio_hi_in);
    }
#endif
}
...
static inline uint32_t gpio_get_all(void) {
    // FIXME RP2350: all of the "mask" / "all" functions are left as is for
    // now. Only the single-GPIO functions have been fixed up for RP2350.
#if PICO_USE_GPIO_COPROCESSOR
...
static inline void gpio_put(uint gpio, bool value) {
    ...
    if (value)
        ...
        gpio_clr_mask(mask);
#elif PICO_USE_GPIO_COPROCESSOR
