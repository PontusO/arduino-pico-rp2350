...
void irq_set_enabled(uint num, bool enabled) {
    ...
    if (enabled) {
        // Clear any previously set IRQ force when the enabling the
        // corresponding IRQ (unlike Cortex-M this does not become set if the
        // IRQ is asserted while disabled, but still worth clearing)
        hazard3_irqarray_clear(RVCSR_MEIFA_OFFSET, num / 16, 1u << (num % 16));
        hazard3_irqarray_set(RVCSR_MEIEA_OFFSET, num / 16, 1u << (num % 16));
    } else {
        hazard3_irqarray_clear(RVCSR_MEIEA_OFFSET, num / 16, 1u << (num % 16));
    }
#else
    // really should update irq_set_mask_enabled?
    needs_love_rp2350_skip();
    if (enabled) {
...
#if defined(__riscv)
    if (enabled) {
        hazard3_irqarray_clear(RVCSR_MEIFA_OFFSET, 0, mask & 0xffffu);
        hazard3_irqarray_clear(RVCSR_MEIFA_OFFSET, 1, mask >> 16);
        hazard3_irqarray_set(RVCSR_MEIEA_OFFSET, 0, mask & 0xffffu);
        hazard3_irqarray_set(RVCSR_MEIEA_OFFSET, 1, mask >> 16);
    } else {
        hazard3_irqarray_clear(RVCSR_MEIEA_OFFSET, 0, mask & 0xffffu);
        hazard3_irqarray_clear(RVCSR_MEIEA_OFFSET, 1, mask >> 16);
    }
#elif PICO_RP2040
    if (enabled) {
        nvic_hw->icpr = mask;
...
#if !PICO_DISABLE_SHARED_IRQ_HANDLERS && !defined(__riscv)
...
#if PICO_RP2350
    // All of the shared IRQ code needs love on RISC-V
...
#if PICO_NO_RAM_VECTOR_TABLE
    panic_unsupported()
#elif PICO_DISABLE_SHARED_IRQ_HANDLERS || defined(__riscv)
...
#if PICO_RP2040
    return (io_rw_32 *)(PPB_BASE + M0PLUS_NVIC_IPR0_OFFSET);
#else
    return (io_rw_32 *) (PPB_BASE + M33_NVIC_IPR0_OFFSET);
#endif
}
#endif
void irq_set_priority(uint num, uint8_t hardware_priority) {
    ...
    hardware_priority = ~hardware_priority >> 4;
    ...
    // todo amy is there anything extra to worry about
...
uint irq_get_priority(uint num) {
    check_irq_param(num);
#ifdef __riscv
    uint16_t priority_row = hazard3_irqarray_read(RVCSR_MEIPRA_OFFSET, num / 4);
    uint8_t priority_4bit = (priority_row >> (4 * (num % 4))) & 0xfu;
    return ~priority_4bit << 4;
#else
