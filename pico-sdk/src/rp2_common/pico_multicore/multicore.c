...
#if !PICO_RP2040
// todo amy this needs to be corrected in headers
// This code refers to PROC0/PROC1 FIFO IRQs, but on platforms other than
// RP2040 this is actually the same *IRQ number*, just with a different SIO
...
#endif
#ifdef __riscv
    // Clear per-IRQ enables, then set global mie.meie + mstatus.mie:
    // (happens during crt0 for core 0)
    extern void init_h3_irq_registers(void);
    init_h3_irq_registers();
#endif
    // todo amy merge this and init_irq_priorites and whatever into a common function to be used by both core inits
...
void multicore_launch_core1_with_stack(void (*entry)(void), uint32_t *stack_bottom, size_t stack_size_bytes) {
    ...
    // todo amy we should support this
...
void multicore_lockout_victim_init(void) {
    ...
    uint fifo_irq_this_core = core_num ? (uint)SIO_IRQ_PROC1 : (uint)SIO_IRQ_PROC0;
...
static bool multicore_lockout_handshake(uint32_t magic, absolute_time_t until) {
    uint irq_num = get_core_num() ? (uint)SIO_IRQ_PROC1 : (uint)SIO_IRQ_PROC0;
