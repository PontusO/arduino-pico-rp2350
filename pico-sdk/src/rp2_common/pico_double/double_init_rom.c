...
#if PICO_RP2350
        // todo amy remove (this is just to allow us to run without float tables when not included in bootrom build)
        if (!rom_table_double || !rom_table_float) {
            for(uint i=0;i<SF_TABLE_V2_SIZE/4;i++) {
                if (!sd_table[i]) sd_table[i] = (uintptr_t)missing_double_func_shim;
            }
            return;
        }
#endif
