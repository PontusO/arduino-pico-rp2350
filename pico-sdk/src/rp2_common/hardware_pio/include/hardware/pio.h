...
#else
    valid_params_if(PIO, pio == pio0 || pio == pio1 || pio == pio2);
#endif
}
...
static inline void sm_config_set_out_pin_base(pio_sm_config *c, uint out_base) {
    valid_params_if(PIO, out_base < NUM_BANK0_GPIOS);
...
static inline void sm_config_set_in_pin_base(pio_sm_config *c, uint in_base) {
    valid_params_if(PIO, in_base < NUM_BANK0_GPIOS);
...
static inline void sm_config_set_in_pin_count(pio_sm_config *c, uint in_count) {
    ...
    ((void)c);
    valid_params_if(PIO, in_count == 32);
#else
    valid_params_if(PIO, in_count && in_count <= 32);
...
static inline void sm_config_set_jmp_pin(pio_sm_config *c, uint pin) {
    valid_params_if(PIO, pin < NUM_BANK0_GPIOS);
...
#if NUM_BANK0_GPIOS > 32 && PICO_PIO_VERSION > 0
...
static inline uint pio_get_index(PIO pio) {
    ...
    return ((uintptr_t)pio - PIO0_BASE) >> 20;
}
...
static inline void pio_gpio_init(PIO pio, uint pin) {
    check_pio_param(pio);
    valid_params_if(PIO, pin < NUM_BANK0_GPIOS);
    gpio_set_function(pin, (enum gpio_function) (GPIO_FUNC_PIO0 + pio_get_index(pio)));
}
...
static inline uint pio_get_dreq(PIO pio, uint sm, bool is_tx) {
    ...
    check_pio_param(pio);
    check_sm_param(sm);
    return sm + (is_tx ? 0 : NUM_PIO_STATE_MACHINES) + pio_get_index(pio) * (DREQ_PIO1_TX0 - DREQ_PIO0_TX0);
}
 ...
 * \param pio The PIO instance; e.g. \ref pio0 or \ref pio1
 * \param program the program definition
 * \return the instruction memory offset the program is loaded at, or negative for error
 */
...
static inline void pio_set_irq0_source_enabled(PIO pio, enum pio_interrupt_source source, bool enabled) {
    check_pio_param(pio);
    invalid_params_if(PIO, source >= 32 || (1u << source) > PIO_INTR_BITS);
    if (enabled)
