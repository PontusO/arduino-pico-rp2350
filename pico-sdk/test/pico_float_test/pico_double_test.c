...
#define EPSILON 1e-9
#define assert_close(a, b) assert((fabs(a - b) < EPSILON) || (isinf(a) && isinf(b) && (a < 0) == (b < 0)))
...
#define check_close1(func,p0) ({ typeof(p0) r = func(p0), r2 = __CONCAT(__real_, func)(p0); printf("%f %f\n", r, r2); if (isnan(p0)) assert_nan(r); else assert_close(r, r2); r; })
#define check_close2(func,p0,p1) ({ typeof(p0) r = func(p0,p1), r2 = __CONCAT(__real_, func)(p0,p1); printf("%f %f\n", r, r2); if (isnan(p0) || isnan(p1)) assert_nan(r); else assert_close(r, r2); r; })
#else
...
//        }
    ...
    for(double x = -4294967296.f * 4294967296.f * 2.f; x<=-0.5f; x/=2.f) {
        printf("d2i64 %f->%lld %lld\n", x, (int64_t)x, __aeabi_d2lz(x));
        if (x < INT64_MIN) {
