...
#define MAIN_RAM_END          0x20082000u // same as SRAM_END in addressmap.h
#define FLASH_START           0x10000000u // same as XIP_MAIN_BASE in addressmap.h
#define FLASH_END             0x12000000u // +32 MiB -- remainder has no external devices mapped
// todo amy based on what sort of elf
#define XIP_SRAM_START        0x13ffc000u // same as XIP_SRAM_BASE in addressmap.h
#define XIP_SRAM_END          0x14000000u // same as XIP_SRAM_END in addressmap.h
...
static int read_and_check_elf32_header(FILE *in, elf32_header& eh_out) {
    ...
    if (eh_out.common.machine != EM_ARM && eh_out.common.machine != EM_RISCV) {
        return fail(ERROR_FORMAT, "Not an Arm or RISC-V executable");
    }
...
int elf2uf2(FILE *in, FILE *out) {
    ...
    if (ram_style) {
        ...
        if (eh.entry == expected_ep_xip_sram && family_id == RP2040_FAMILY_ID) {
           return fail(ERROR_INCOMPATIBLE, "RP2040 B0/B1/B2 Boot ROM does not support direct entry into XIP_SRAM\n");
        } else if (eh.entry != expected_ep && family_id == RP2040_FAMILY_ID) {
            return fail(ERROR_INCOMPATIBLE, "A RP2040 RAM binary should have an entry point at the beginning: %08x (not %08x)\n", expected_ep, eh.entry);
        }
...
struct {
    ...
    uint32_t family_id;
        ...
        { "rp2350-arm-ns", RP2350_ARM_NS_FAMILY_ID },
        { "rp2350-riscv", RP2350_RISCV_FAMILY_ID },
        { "absolute", ABSOLUTE_FAMILY_ID },
        { "data", DATA_FAMILY_ID },
};
...
int main(int argc, char **argv) {
    ...
    // todo this should be handled by looking at the IMAGE by default
    if (arg < argc && !strcmp(argv[arg], "-f")) {
        arg++;
        if (arg >= argc) return usage();
        int index = -1;
        for(int i=0;i<count_of(families);i++) {
            if (!strcmp(argv[arg], families[i].name)) {
                index = i;
                break;
            }
        };
